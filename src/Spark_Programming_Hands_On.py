...
dbutils.widgets.text("write_schema", "_silver", "write_schema ( Your name ie. jack_gibb)")
...
display(spark.sql(f"""DESCRIBE EXTENDED {catalog}.{write_schema}.customer_transaction_summary"""))

# COMMAND ----------

spark.sql(f"""
ALTER TABLE {catalog}.{write_schema}.customer_transaction_summary
ALTER COLUMN customer_id
COMMENT 'Primary Key - The ID of the Customer'""")

# COMMAND ----------

display(spark.sql(f"""DESCRIBE HISTORY {catalog}.{write_schema}.customer_transaction_summary"""))

# COMMAND ----------

# -- Roll the table back to the version written before the append
spark.sql(f"""RESTORE TABLE {catalog}.{write_schema}.customer_transaction_summary TO VERSION AS OF 0""")
...
# cache() / persist() store a DataFrame's partitions in executor memory so repeated access
# skips recomputation. Only fully cached once an action touches every record (e.g. count()).
transactions_cached = transactions.cache()
transactions_cached.count()  # materialises the full cache

# COMMAND ----------

# persist(StorageLevel.X) gives fine-grained control over how data is cached
repayments_persisted = repayments.persist(StorageLevel.MEMORY_AND_DISK)
repayments_persisted.count()  # materialises the full persist

# Release the cache/persist once no longer needed
transactions_cached.unpersist()
repayments_persisted.unpersist()
